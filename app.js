...
const AUTH_STORAGE_KEY = "record_oppa_ninja_session";
const OWNER_USER = {
  id: "owner",
  name: "Owner",
  role: "owner"
...
function bindEvents() {
  elements.ownerLoginForm.addEventListener("submit", handleOwnerCodeLogin);
...
async function handleOwnerCodeLogin(event) {
  event.preventDefault();
  elements.authAlert.textContent = "";
  elements.authAlert.classList.remove("success");

  const ownerCode = elements.ownerCodeInput.value.trim();
  if (!ownerCode) {
    elements.authAlert.textContent = "Masukkan kod owner.";
    return;
  }
...
function updateAuthView() {
  ...
  elements.currentUserName.classList.toggle("owner-user", isLoggedIn);

  if (!isLoggedIn) {
    window.setTimeout(() => elements.ownerCodeInput.focus(), 0);
  }
}
...
function getRequestAuth() {
  const session = getSession();
  return { ownerCode: session?.role === "owner" ? session.ownerCode || "" : "" };
}
...
function formatUserLabel(profile) {
  return `${profile?.name || "Owner"} (Owner)`;
}
